...
class CauseCodeProcessor(IEntityProcessor):
    ...
    def get_from_db(self, conn, get_ref_value=False):
        ...
        if not res:
            raise RuntimeError("Unable to retrieve Cause Code from Database")
        else:
    ...
    def process_file_import(self, source_df, conn, catch_failed=True):
        ...
        causeCode_df = source_df.rename(self.columnNameMap, axis=1)
        causeCode_df.fillna('', inplace=True)

        database_df = self.get_from_db(conn)
        causeCode_df_new = pd.merge(causeCode_df, database_df, on=['CauseCode', 'CauseText'], how='outer',
                                    indicator=True) \
            .query("_merge == 'left_only'") \
            .drop(columns=['_merge', 'Id'])

        if not causeCode_df_new.empty:
            print(f'Identified {len(causeCode_df_new)} new records for Cause codes')
            affected, errors, failed = self.__db_handler__.write_to_db(conn=conn, schema=self.__db_schema__,
                                                                       table_name=self.__db_table_name__,
                                                                       dataFrame=causeCode_df_new,
